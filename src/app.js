...
try {
  // citys
  let delhi = await citysmodel.create({ cityname: "delhi" });
  let mumbai = await citysmodel.create({ cityname: "mumbai" });
  let bangalore = await citysmodel.create({ cityname: "bangalore" });
  let chennai = await citysmodel.create({ cityname: "chennai" });
  let pune = await citysmodel.create({ cityname: "pune" });
  //cars
  let bmw = await carmodel.create({ carname: "bmw" });
  let audi = await carmodel.create({ carname: "audi" });
  let hyundai = await carmodel.create({ carname: "hyundai" });
  let honda = await carmodel.create({ carname: "honda" });
  let tesla = await carmodel.create({ carname: "tesla" });
  let kia = await carmodel.create({ carname: "kia" });
